...
import { getUserSetsWithFlashcards, FlashcardSet, Flashcard } from "@/lib/flashcards";

export default function MyFlashcardList() {
  ...
  const fetchData = async () => {
    ...
    const setsData = await getUserSetsWithFlashcards(user.id);
    setSets(setsData ?? []);
    setLoading(false);
  };
    ...
    <div className="absolute top-0 left-64 right-0 p-6 pl-14 pr-14 flex flex-col gap-6 items-center font-kaisei text-[#3a3c4e]">
      ...
      <div className="w-full h-full rounded-lg p-7 bg-[#cddfd1]">
        {/* Show if no sets exist */}
          {sets.length === 0 && (
            <p className=" text-center">No sets yet.</p>
          )}
